...
public class LibraryManager : MonoBehaviour
{
    ...
    IEnumerator WaitThenBuild()
    {
        ...
        // 如果 PlayerDataManager 存在并且可能尚未加载完数据，订阅 OnPlayerDataLoaded，以便数据就绪时重建一次
        if (playerDataManager != null)
        {
            try
            {
                ...
                handler = new Action(() =>
                {
                    ...
                    {
                        try { playerDataManager.OnPlayerDataLoaded -= handler; } catch { }
                    }
                });

                // 订阅（安全，如果事件已触发过则不会再触发，但我们仍会后续主动尝试构建）
                playerDataManager.OnPlayerDataLoaded += handler;
            }
            catch { }
        }

        // 尝试构建（若数据尚未就绪，会回退到 CSV 或卡表逐项查询）
        BuildLibraryFromPlayerData();
        yield break;
    }
    ...
    void BuildLibraryFromPlayerData()
    {
        ...
        // 优先：直接使用 PlayerDataManager.playerCards（库存）
        if (playerDataManager != null)
        {
            try
            {
                var pd = playerDataManager;
                if (pd.playerCards != null && pd.playerCards.Count > 0)
                {
                    playerCounts = new Dictionary<int, int>(pd.playerCards);
                    Debug.Log($"[LibraryManage] 使用 PlayerDataManager.playerCards（count={playerCounts.Count}）作为数据源");
                }
                else
                {
                    // 如果 playerCards 为空，尝试通过 cardStore 中定义的卡片逐个读取 GetCardCount（更稳妥）
                    if (cardStore != null)
                    {
                        try
                        {
                            var csType = cardStore.GetType();
                            FieldInfo fl = csType.GetField("cardList", BindingFlags.Public | BindingFlags.Instance)
                                         ?? csType.GetField("cardData", BindingFlags.Public | BindingFlags.Instance);

                            if (fl != null)
                            {
                                var listObj = fl.GetValue(cardStore) as System.Collections.IEnumerable;
                                if (listObj != null)
                                {
                                    foreach (var item in listObj)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            var pid = itType.GetProperty("Card_ID") ?? itType.GetProperty("CardId") ?? itType.GetProperty("ID");
                                            if (pid != null) id = Convert.ToInt32(pid.GetValue(item));
                                            else
                                            {
                                                var fid = itType.GetField("Card_ID") ?? itType.GetField("CardId") ?? itType.GetField("ID");
                                        ...
                                        if (id >= 0)
                                        {
                                            int cnt = playerDataManager.GetCardCount(id);
                        ...
                        {
                            Debug.LogWarning($"[LibraryManage] 通过 cardStore 逐卡读取时异常: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                Debug.LogWarning($"[LibraryManage] 从 PlayerDataManager 读取 playerCards 时异常: {ex.Message}");
            }
        }

        // fallback: 如果仍为空，尝试使用 Inspector 的 playerDataCsv 文本
        if ((playerCounts == null || playerCounts.Count == 0) && playerDataCsv != null)
        {
            playerCounts = ParsePlayerDataCsvSimple(playerDataCsv.text);
            Debug.Log($"[LibraryManage] 使用 playerDataCsv 作为回退，解析到 cardCount={playerCounts.Count}");
        }

        try { CardCounter.SetPlayerCounts(playerCounts); } catch { }

        if (playerCounts == null || playerCounts.Count == 0)
        {
    ...
    void TryPersistToPlayerDataManager(int cardId, int newCount)
    {
        ...
        {
            // 使用明确接口写回库存（不自动保存）
            playerDataManager.SetCardCountNoSave(cardId, newCount);
            Debug.Log($"[LibraryManage] 写回 playerCards via SetCardCountNoSave id={cardId} count={newCount}");
        }
        catch (Exception ex)
        {
